...
Tetromino::Tetromino() : active(false), id(0) {}

Tetromino::Tetromino(const Tetromino &argTetromino) {
...
Tetromino::Tetromino(Pixel coorZero, int argId) : active(true), id(argId) {
    pixels.resize(amount_of_pixels);
    int random = argId;
    for (unsigned i = 0; i < pixels.size(); i++) {
        pixels[i].setX(-1);
        pixels[i].setY(-1);
        pixels[i].setZ(-1); // Initialize Z coordinate
    }
    ...
    for (unsigned i = 1; i < pixels.size(); i++) {
        if (pixels[i].getX() == -1 && pixels[i].getY() == -1 && pixels[i].getZ() == -1) {
            pixels[i].setX(pixels[0].getX());
            pixels[i].setY(pixels[0].getY());
            pixels[i].setZ(pixels[0].getZ());
        }
    }
}

std::vector<Pixel> Tetromino::getPixels() const {
    return pixels;
}

std::string Tetromino::getColor() const {
    return color;
}
...
void Tetromino::setPixels(const std::vector<Pixel>& argPixels) {
